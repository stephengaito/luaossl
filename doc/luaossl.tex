...
\subsubsection[\fn{pkey:encrypt}]{\fn{pkey:encrypt($string$)}}

Encrypt the plain text string. Note that pkey:encrypt only works for RSA public keys.

Returns the encrypted text as a lua byte string on success, and throws an error otherwise.

\subsubsection[\fn{pkey:decrypt}]{\fn{pkey:decrypt($string$)}}

Decrypt the encrypted lua byte string. Note that pkey:decrypt only works for RSA private keys.

Returns the decrypted text as a lua string on success, and throws an error otherwise.
